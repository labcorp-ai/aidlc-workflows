...
Create the intent directory structure per `.claude/conventions/folder-structure.md`:
...
5. Initialize `state/state.json` per `.claude/conventions/state-schema.json` (empty stages array)
6. Initialize `audit/audit.json` per `.claude/conventions/audit-schema.json` (empty entries array)
