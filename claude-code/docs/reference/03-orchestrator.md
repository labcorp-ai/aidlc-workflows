...
| Stage Progress | Per-stage checkboxes organized by phase (see below) |
...
2. **Enter the gate**: `bun .claude/tools/aidlc-state.ts gate-start <slug>`. Marks `[-]` → `[?]`, emits `STAGE_AWAITING_APPROVAL`, makes `/aidlc --status` show "Awaiting your approval on \<stage\>".

3. **Present the approval gate** (AskUserQuestion). Under `--test-run`, skip the prompt.

4. **Record the user's response**:
   - **Approve** → `bun .claude/tools/aidlc-state.ts approve <slug> --user-input "<exact choice>"` (add `--test-run` under test-run). Emits `GATE_APPROVED` + `STAGE_COMPLETED` atomically, marks `[?]` → `[x]`.
