...
1. **Kickoff** — read `.claude/skills/aidlc-kickoff/SKILL.md`. Welcome the human, set up the workspace.
2. **Workflow Composition** — read `.claude/skills/aidlc-workflow-composition/SKILL.md`. Compose the adaptive workflow conversationally with the human.
3. **Stage Execution** — read `.claude/skills/aidlc-stage-execution/SKILL.md`. Drive each stage through its cycle.
...
Read and follow all files in `.claude/conventions/`. They define the folder structure, state format, audit format, and workflow format.
