...
//   1 grep STATE 'IDEATION'            -> readStateField(state,"Lifecycle Phase") === "IDEATION"
//                                         (jump.ts:312 setField targetStage.phase.toUpperCase()).
//   2 assert_lt COMPLETED 20           -> readStateField(state,"Completed") parsed-int < 20 (the
//                                         .sh's EXACT surface + bound). The jump reset the 20
//                                         downstream EXECUTE stages (jump.ts:268-285); without
//                                         --test-run nothing re-completes them, so the on-disk
//                                         Completed at rest is the jump value (probe observed 4).
//                                         We keep the .sh's loose < 20 bound here and pin the EXACT
//                                         destination on the now-stable Current Stage field below.
//   3 grep AUDIT 'intent-capture'      -> (a) raw audit.md bytes contain "**Target**: intent-capture"
//                                         (jump.ts:377 Target field), the append-only deterministic
//                                         origin of the destination; AND (b) — NEW, STRONGER than the
//                                         old port — readStateField(state,"Current Stage") ===
//                                         "intent-capture", the now-stable post-jump live field. With
//                                         --test-run gone, the run STOPS at the target, so the live
//                                         Current Stage no longer auto-advances and is directly
//                                         assertable (probe-verified).
