...
//     default, but `/aidlc feature` overrides it -> Scope=feature on disk.
//     .sh assertion ported: state `- **Scope**: feature`.
//   - Case known-scope positional: `/aidlc feature` bootstraps Scope=feature
//     without a disambiguation gate; the bare known-scope token wins over the
...
// rendered statusline — never racy terminal completion (the Phase-2 lesson). No
// gate is answered; the scope-resolution + state-init is the whole surface.
//
...
// bare `--tui` SKIPs it; tmux/claude/distributable absence also SKIPs with a
...
// bare --tui (no live opt-in) reports a clear skip reason, not a substrate miss.
...
describe("t-tui-t29 env-scope (AWS_AIDLC_DEFAULT_SCOPE seeds new-workflow scope on disk)", () => {
  ...
  test.skipIf(SKIP_REASON !== null)(
    ...
    () => {
      ...
      try {
        ...
        expect(pane).toContain("Start a workflow with /aidlc");
        expect(pane).toContain("or scaffold one with");
  ...
  // --- Case override: EXPLICIT --scope flag wins over env, SILENTLY -----------
  ...
  // env path: no env synthesis, no disambiguation, Scope=feature lands silently.
  // VERIFIED LIVE 2026-06-06: `/aidlc --scope feature` (workshop env present) ->
  // straight to IDEATION/intent-capture, Scope=feature, NO gate. (Contrast the
  ...
  // contract.)
  test.skipIf(SKIP_REASON !== null)(
    `explicit --scope feature flag wins over env default -> Scope=feature on disk, no gate${SKIP_REASON ? ` — SKIP: ${SKIP_REASON}` : ""}`,
    async () => {
      ...
      try {
        ...
        const landed = await waitForDisk(() => scopeLanded(proj, "feature"), 300000);
        const pane = drive(["capture", "--session", session]).stdout;
        if (!landed) {
          throw new Error(
  ...
  // Current v0.6.1 behaviour treats `/aidlc feature` as a known-scope request.
  // The first read-only engine call reports no state; the TUI conductor then
  // uses the bootstrap path and initializes feature. No disambiguation AUQ is
  // expected or required.
  test.skipIf(SKIP_REASON !== null)(
    `bare known-scope command (/aidlc feature) bootstraps Scope=feature without a disambiguation gate${SKIP_REASON ? ` — SKIP: ${SKIP_REASON}` : ""}`,
    async () => {
