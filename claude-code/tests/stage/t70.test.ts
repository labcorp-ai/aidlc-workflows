...
//
// t70.test.ts — SDK-harness port of tests/stage/t70-stage-workspace-detection-greenfield.sh
// (plan 8). Drives the real /aidlc --init --force --test-run against a project
// seeded with the greenfield-todo stub (a bare README.md, no source/manifest/
// framework config) and a pre-seeded state pinned at workspace-detection. Asserts
// ONLY on deterministic surfaces — on-disk state fields, audit events, and the
// init tool's verbatim Bash stdout — NEVER on assistantText.
//
...
// exit semantics. The whole init path is deterministic: SKILL.md:531 routes
// `--init --force --test-run` to `bun .claude/tools/aidlc-utility.ts init
// --scope <scope> --arguments "$ARGUMENTS" --test-run` via Bash and prints its
// stdout verbatim. handleInit (aidlc-utility.ts:1716) removes the seeded state on
...
describe("t70 /aidlc --init --force --test-run, greenfield stub (sdk)", () => {
  // -------------------------------------------------------------------------
  ...
  test(
    ...
    async () => {
      ...
      // an audit log. --force makes init run on the seeded state; --test-run
      // auto-confirms any prompt (the init path has none — SKILL.md:138 prints
      // state and STOPs).
      ...
      try {
        const r = await driveAidlc("/aidlc --init --force --test-run", {
          projectDir: proj,
        ...
        // em-dash placeholder").
        assertStateField(r, "Project Root", proj);
