...
`.claude/aidlc-common/stages/<phase>/<slug>.md`. The phase is the directory.
There are five:

```
.claude/aidlc-common/stages/
├── initialization/
├── ideation/
├── inception/
├── construction/
...
### 4. Compile so `stage-graph.json` regenerates

The YAML you just authored is the authoritative source. The runtime reads a
compiled artifact, `.claude/tools/data/stage-graph.json`, produced from the
YAML by the graph compiler. After editing stage YAML, recompile:

```bash
bun .claude/tools/aidlc-graph.ts compile
```

This regenerates `stage-graph.json` from every stage file. The authoring flow
is a one-way pipeline — edit YAML, run `compile`, the JSON updates, and the
...
`/aidlc-init` command instead, wrapping `/aidlc --init`.) After adding (or removing)
