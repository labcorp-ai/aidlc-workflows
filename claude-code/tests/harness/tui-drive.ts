...
//
// CANNOT BE VALIDATED IN THIS SESSION (no Windows host). Written faithfully to
// the spike (tmp/capture-v3.sh, tmp/bunvsnode2.sh, vault "node-pty Input
// Requires Node Not Bun on Windows ConPTY"); live validation DEFERRED to the
// EC2 box i-0d8daa430102039a1.
//
...
async function cmdStart(backend: Backend, a: Args): Promise<void> {
  ...
  writeTuiTrace(session, "start", { cwd, width, height, command: a.rest });
  await backend.start(session, cwd, width, height, a.rest);
}
...
// Count++); option 1 silently records approval and the reject never takes. This
// returns the option number of the FIRST genuine revision directive — the
...
function pickRevisionOption(grid: string): number | null {
  if (!gridHasMenu(grid)) return null;
  const NON_REVISE =
    /(actually approve|approve & continue|approve and continue|didn't mean to reject|cancel|type something|chat about|nevermind|never mind)/i;
...
async function cmdAnswerGate(backend: Backend, a: Args): Promise<void> {
  ...
  // 2026-06-07). Keyed on the literal option label "Request Changes"
  ...
  for (;;) {
    ...
    if (gridIsSubmitScreen(grid)) {
    ...
    } else if (rejectFirstGate && grid.includes("Request Changes")) {
      writeTuiTrace(session, "answer_gate_action", {
        answered,
      ...
      // 2026-06-09). Two shapes are possible:
      //   A) RECOVERY MENU: `1. Actually approve & continue` (UN-rejects!) then
      //      real revise directives (`Narrow root cause…`, `Drop … steer`, `Add
      //      more detail`). Selecting a genuine revise option is what triggers
      //      `reject --feedback` (Revision Count++). We pick the first non-escape
      //      option by NAVIGATING the caret to it (Down × (num-1) from option 1),
      //      never option 1 and never the type/chat trailers.
      //   B) FREE-TEXT prompt (older engines / no recovery menu): type concrete
      //      revision feedback as a literal keystroke + Enter.
      // The orchestrator then re-runs the stage and re-presents the SAME approval
      // gate, which the (now approve-only) loop approves on its next pass —
      // completing the reject→revise→approve cycle. Poll briefly for whichever
      // shape settles so keystrokes land in the right place, not mid-stream.
      ...
      while (Date.now() < recoveryDeadline) {
        ...
        if (!gridHasMenu(after) && Date.now() > recoveryDeadline - 50_000) {
          backend.send(
            session,
            "Narrow the analysis to the checkbox/completed-flag persistence specifically.",
            true,
            true,
          );
          await sleep(300);
