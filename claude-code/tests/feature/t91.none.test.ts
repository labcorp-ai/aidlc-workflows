...
//
// Mechanism = none. Port of tests/feature/t91-runtime-compile-hook.sh (TAP
...
//   2. Command filter: `\bbun\b.*\.claude/tools/aidlc-(state|jump|bolt|
//      utility)\.ts\b` must match AND `aidlc-runtime\.ts` must NOT appear
//      (the explicit recursion-guard reject fires FIRST, defeating composites).
...
//
// 13 .sh asserts -> 13 expect()-bearing test() cases here.
