...
//
// t53.test.ts — SDK-harness port of tests/workflow/t53-workflow-scope-routing.sh
// (plan 11). Drives the real `/aidlc bugfix --test-run` through the Claude Agent
// SDK until the deterministic orchestrator directive is observed, then asserts
// ONLY on deterministic surfaces (tool_result JSON, on-disk state fields, the
// state file's Scope-Configuration / Stage-Progress blocks, and the parsed
// + raw audit) — NEVER on assistantText.
//
...
// post-init stage. The orchestrator routes `bugfix --test-run` through
// `aidlc-utility.ts init --scope bugfix --test-run` (SKILL.md:531), which writes
...
//   - --test-run init dispatch: SKILL.md:531-533
//
// It SPENDS TOKENS — driveAidlc drives the real /aidlc bugfix --test-run on
...
// ---------------------------------------------------------------------------
// Timeout budget. `/aidlc bugfix --test-run` is live SDK traffic even though the
...
function seedBugfixState(proj: string): void {
  ...
  const res = spawnSync(
    process.execPath,
    [utility, "init", "--scope", "bugfix", "--project-dir", proj, "--test-run"],
...
describe("t53 /aidlc bugfix --test-run scope routing (sdk)", () => {
  test(
    ...
    async () => {
      ...
      try {
        ...
        const r = await driveAidlc("/aidlc bugfix --test-run", {
          projectDir: proj,
        ...
        // stages [x] at init; the EXECUTE stages auto-advance under --test-run
        // but cannot exceed the 6 in-scope stages. The .sh bounded this < 12.
