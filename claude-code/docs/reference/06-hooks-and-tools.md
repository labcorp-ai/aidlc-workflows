...
| `aidlc-stop.ts` | Stop | Project-wide (settings.json) | (empty) | **Flow-altering.** Enforce the forwarding loop on turn-end: run `aidlc-orchestrate next`; on `done` allow the stop, on a pending directive block the stop and inject the next move back via `reason`. Recursion-bounded (no-progress counter + `stop_hook_active` under `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`, default 8). No-op outside an AIDLC workflow |
...
5. **Pending → block and inject:** For any other (pending) directive — `run-stage`, `dispatch-subagent`, `invoke-swarm`, `present-gate`, `ask`, `print`, `error` — it prints `{"decision":"block","reason":<on-task continuation>}`, so the same session resumes with the next move injected.
6. **Fail open:** Any unexpected failure (unreadable state, an engine that exits non-zero or returns no parseable directive, malformed stdin) allows the stop and records a drop. Failing open is the only safe failure mode for a hook that can otherwise trap a turn.
...
**Human interrupt is free.** Stop hooks do not fire on user interrupt (Esc), so a manual interrupt can never be trapped — no code is needed for that case.
...
| **Workflow Lifecycle** | 2 | `WORKFLOW_STARTED`, `WORKFLOW_COMPLETED` | `aidlc-utility.ts init`, `aidlc-state.ts complete-workflow` |
...
| **Navigation** | 5 | `SCOPE_CHANGED`, `SCOPE_DETECTED`, `DEPTH_CHANGED`, `TEST_STRATEGY_CHANGED`, `TEST_RUN_MODE_ENABLED` | `aidlc-utility.ts` |
...
**Test-run events:** The `--test-run` flag on emitting tools (`aidlc-state.ts approve --test-run`, `aidlc-log.ts answer --test-run`, `aidlc-jump.ts execute --test-run`) tags the canonical event with `Test-Run: true`. There is no separate auto-event category — the tag preserves observability without taxonomy bloat.
...
| `init` | Run the Initialization phase (scaffold dirs, detect workspace, init state). Accepts `--scope <scope>` (defaults to `poc`), `--depth`, `--test-strategy`, `--test-run`, `--force`. | `WORKFLOW_STARTED`, `PHASE_STARTED`, `PHASE_SKIPPED`, `STAGE_STARTED`, `STAGE_COMPLETED`, `WORKSPACE_*`, and the init→first-post-init phase hand-off events |
...
| `enable-test-run` | Persist Test Run Mode flag in state file on resume | `TEST_RUN_MODE_ENABLED` |
...
| Subcommand | Purpose | Emits |
|------------|---------|-------|
| `compile [--test-run]` | Walk audit + memory, rewrite `runtime-graph.json`; emit a `MEMORY_EMPTY` row per approved stage whose diary is empty | `MEMORY_EMPTY` |
...
- [Claude Code Features](14-claude-features.md) -- settings.json configuration
