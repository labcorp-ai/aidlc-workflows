...
// dir (toPortablePath-wrapped — doctor WRITES GUARDRAIL_LOADED + HEALTH_CHECKED
// audit rows under <proj>/aidlc-docs, and the tool resolves that path through
// forward-slash helpers, so a native-Windows path must round-trip). NOTHING
// is written under tests/fixtures/**; all rule files are built inline (the
// .sh's L1 rationale — too combinatorial for an on-disk fixtures dir). All
