// covers: audit:SENSOR_FIRED audit:WORKSPACE_INITIALISED
//
...
// tool-emitted audit.
//
// WHAT A HARNESS ENGINEER DOES (docs/harness/00-overview.md): reshape the
...
// what artefacts get produced, bound sensors, and rules. The customHarness
...
// `schema-validator` sensor and a unique project-rule marker. Note the slug≠
// artefact split (shipped convention: the stage is the ACTIVITY, the artefact is
// the named DELIVERABLE). "Harness Engineer" is the PERSONA; data-migration is
// the realistic domain workflow they define.
//
...
//   4. CUSTOM STAGE RENDERS IN THE STATUSLINE — tui: the painted statusline
//      shows "> schema-snapshot". FINDING (surfaced, not softened): the
//      statusline STAGE_DISPLAY map is hardcoded to the 32 framework slugs
//      (aidlc-statusline.ts:62-95) and renders STAGE_DISPLAY[stage] ?? stage
//      (:220); stages carry no display_name (that's agents). So a CUSTOM stage
//      renders its RAW SLUG, not a pretty display name — the plan's "display
//      name renders" is achievable only as "raw slug renders". The cheapest,
//      token-free proof of the statusline surface is a SEEDED-STATE capture
//      (mirrors t-tui-render-statusline), asserted here.
//
...
// ABSENT / opt-in gating, AIDLC_TUI_LIVE first (a bare --tui run reports the
...
describe("t-tui-custom-harness (the {sdk,tui} two-driver journey)", () => {
  ...
  // the painted statusline renders "> schema-snapshot" — the custom stage's RAW
  // SLUG (the hardcoded STAGE_DISPLAY map has no entry, so it falls through; see
  // the header FINDING). Mirrors t-tui-render-statusline.serial.tui.test.ts.
  // -------------------------------------------------------------------------
  test.skipIf(SKIP_REASON !== null)(
    `statusline renders "> ${SNAPSHOT_STAGE_SLUG}" (raw slug — custom stage has no STAGE_DISPLAY entry)${SKIP_REASON ? ` — SKIP: ${SKIP_REASON}` : ""}`,
    () => {
