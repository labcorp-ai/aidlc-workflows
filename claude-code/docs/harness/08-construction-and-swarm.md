# Shaping Autonomous Construction
...
Construction by editing **data** under `.claude/` — a rule, a stage, a sensor's
...
|---|---|---|
| The team's autonomy **posture** (a standing default) | you, the harness engineer | a rule in `.claude/rules/aidlc-{team,project}.md` (data) |
...
demands. The shipped default lives in the org rule at
`.claude/rules/aidlc-org.md` under the `## Walking Skeleton` heading
...
bullet under `## Walking Skeleton` in `.claude/rules/aidlc-team.md`:
...
(`dist/claude/.claude/aidlc-common/stages/inception/units-generation.md`
...
(`dist/claude/.claude/hooks/aidlc-stop.ts:74`, exposed as
...
protected spec, the complementary sensor — is data you author under `.claude/`
