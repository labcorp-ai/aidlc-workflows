...
A real stage lives at `dist/claude/.claude/aidlc-common/stages/<phase>/<slug>.md` — for example `stages/inception/application-design.md`. Open one alongside this chapter; the shape will be familiar by the end.
...
- **`lead_agent` and `support_agents` validate against `.claude/agents/*.md`.** There's no hardcoded list — adding an agent means dropping its file in that directory (see [Adding an Agent](03-adding-an-agent.md)).
...
- **`## Learn`** documents the learning-loop ritual — the `memory.md` diary the agent keeps while the stage runs, and how kept observations route into rules and sensors at the approval gate. Crucially, this ritual writes into the *harness* (`.claude/rules/`, `.claude/sensors/`), never back into the stage file itself.
