...
that runs — the feedback half of the harness to the rules' feedforward half. A
...
`.claude/sensors/`. The frontmatter is a pure **capability descriptor** — it
...
**1. Drop a manifest at `.claude/sensors/aidlc-<id>.md`.** The filename stem
...
stage. Name your sensor `.claude/sensors/aidlc-<id>.md` and set `id: <id>`; the
