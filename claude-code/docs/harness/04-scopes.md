...
For a harness engineer, a scope is pure data, authored the same way every other primitive is — as a file. It is two halves: one `.claude/scopes/aidlc-<name>.md` file (its identity — name, depth, keywords, description) plus a per-stage membership tag (each stage's frontmatter `scopes:` list naming the scopes it runs under). Adding or tuning a scope requires no TypeScript. This chapter walks the workflow: what a scope is made of, how to add a team scope, how to tune an existing one, and what the tooling checks for you versus what it leaves to you.

For the full nine-scope catalog with use cases and the routing table users read, see [Scopes, Depth, and Test Strategy](../guide/04-scopes-and-depth.md) in the User Guide. This chapter is the authoring side of that same data.
...
**1. The scope file — `.claude/scopes/aidlc-<name>.md`.** One file per scope, mirroring `.claude/sensors/`. The `feature` scope's frontmatter looks like this:
...
**2. The membership tag — each stage's `scopes:` frontmatter.** A stage names the scopes it runs under in its own frontmatter, in `.claude/aidlc-common/stages/<phase>/<slug>.md`:
...
1. **Drop `.claude/scopes/aidlc-hotfix.md`.** Copy `aidlc-bugfix.md` (the closest existing scope) and edit the frontmatter: set `name: hotfix`, pick `depth`, add `keywords` if you want freeform auto-detection (`[hotfix, urgent]`), a `description` for the help text, and `testStrategy` only if it should diverge from `depth`. Write a short prose body explaining the intent.

2. **Tag the stages that should run under `hotfix`.** In each stage you want `EXECUTE` (under `.claude/aidlc-common/stages/<phase>/`), add `hotfix` to its frontmatter `scopes:` list. A stage you don't tag is `SKIP` for the scope. The 3 initialization stages must include it (they always run).
