...
// scope-start journeys (t50 bugfix / t51 poc / t29 env-scope already drive the
// scope-start surface). Folding it here would mix two journeys in one file and
// pull in answer-gate territory. The config-change one-shot (Cases A + C) is the
// journey t27 names; Case B's depth-at-start is covered structurally by the
// scope-start ports. FLAGGED for the live-verify pass: if the registry wants
// t27 to also credit the scope-start-depth surface, that is a SEPARATE Pattern-A
// it() and should be added to whichever scope-start file owns the start surface,
// not bolted on here.
//
...
// AIDLC_TUI_LIVE=1 so a bare `--tui` on a laptop SKIPs; tmux/claude/distributable
